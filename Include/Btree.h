...
public:
  ...
  Get(const std::string &key) const; // 新增的Get函数声明
  std::vector<std::pair<std::string, std::string>>
  GetRange(const std::string &left_key, const std::string &right_key) const;
  bool Empty() const;
  size_t Size() const;
...
private:
  template <typename T> T *Map(off_t offset) const;
  template <typename T> void UnMap(T *map_obj) const;
  template <typename T> T *Alloc();
  template <typename T> void Dealloc(T *node);
