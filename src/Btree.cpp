...
public:
  ...
  template <typename T> void Put(T *block) {
    while (size_ > kMaxCacheSize)
      Kick();
  ...
  template <typename T> T *Get(int fd, off_t offset) {
...
private:
  ...
  struct Node {
    Node()
        : block(nullptr), offset(0), size(0), ref(0), prev(nullptr),
          next(nullptr) {}

    Node(void *block_, off_t offset_, size_t size_)
        : block(block_), offset(offset_), size(size_), ref(1), prev(nullptr),
          next(nullptr) {}
...
std::optional<std::string> BPlusTree::Get(const std::string &key) const {
  // 检查输入key的有效性
  if (key.empty() || key.length() >= kMaxKeySize) {
    return std::nullopt;
  }

  // 获取叶子节点的偏移量
  off_t offset = GetLeafOffset(key.c_str());
  if (offset == 0) {
    return std::nullopt;
  }

  // 映射叶子节点
  LeafNode *leaf_node = Map<LeafNode>(offset);
  if (!leaf_node) {
    return std::nullopt;
  }

  // 在叶子节点中查找key
  int index = GetIndexFromLeafNode(leaf_node, key.c_str());

  // 如果找不到key,返回空的optional
  if (index < 0) {
    UnMap(leaf_node);
    return std::nullopt;
  }
  ...
  // 找到key,返回对应的value
  std::string value(leaf_node->records[index].value);
  UnMap(leaf_node);
  return std::make_optional(value);
}

template <typename T> T *BPlusTree::Map(off_t offset) const {
  return block_cache_->Get<T>(fd_, offset);
}

template <typename T> void BPlusTree::UnMap(T *map_obj) const {
...
template <typename T> T *BPlusTree::Alloc() {
...
template <typename T> void BPlusTree::Dealloc(T *node) { UnMap<T>(node); }
...
BPlusTree::GetRange(const std::string &left_key,
